...
def main():
    ...
    if(op==2):
        ...
        print(res.count())
    if(op==3):
        res=alunos.find({"dependencias":{"$exists":True},"RA":{"$regex":"^0"},"nome":{"$regex":"^a"},"projeto":{"$exists":False},"$or":[{"CR":{"$gt":0.3}}, {"CR":{"$lt":0.9}},{"semestre":1},{"dependencias":{'$size':5}}],"dependencias":"Banco de Dados 1"})
        print(res.count())
